 ...
 * 
 */
public class ManagmentDurCFSendMailSender extends MailSender
{

	/**
	 * @param info
	 */
	public ManagmentDurCFSendMailSender(List<InfoObject> info)
	{
		super(info);
	}
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getManagmentDurCFSendSubjectSettings(session).getValue());
		mailEnt.setMessage(TextCoder.Encode(MailHelper.getInstance()
				.completeMessageManagmentDur(
						BeansFactory.MailSettings()
								.getManagmentDurCFSendTemplateSettings(session)
								.getValue(), projectName, number,
						durComplitationDate, totalAmountOfDur, certifyDate)));
