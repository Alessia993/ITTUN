 ...
 * 
 */
public class ManagmentDurUCApproveMailSender extends MailSender
{

	/**
	 * @param info
	 */
	public ManagmentDurUCApproveMailSender(List<InfoObject> info)
	{
		super(info);
	}
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getManagmentDurUCApproveSubjectSettings(session).getValue());
		mailEnt.setMessage(TextCoder.Encode(MailHelper.getInstance()
				.completeMessageManagmentDur(
						BeansFactory
								.MailSettings()
								.getManagmentDurUCApproveTemplateSettings(
