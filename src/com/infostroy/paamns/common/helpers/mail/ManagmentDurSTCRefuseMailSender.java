 ...
 * 
 */
public class ManagmentDurSTCRefuseMailSender extends MailSender
{

	/**
	 * @param info
	 */
	public ManagmentDurSTCRefuseMailSender(List<InfoObject> info)
	{
		super(info);
	}
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getManagmentDurSTCRefuseSubjectSettings(session).getValue());
		mailEnt.setMessage(TextCoder.Encode(MailHelper.getInstance()
				.completeMessageManagmentDur(
						BeansFactory
								.MailSettings()
								.getManagmentDurSTCRefuseTemplateSettings(
