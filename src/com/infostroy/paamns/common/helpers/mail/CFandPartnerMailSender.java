...
public class CFandPartnerMailSender extends MailSender
{

	/**
	 * @param info
	 */
	public CFandPartnerMailSender(List<InfoObject> info)
	{
		super(info);
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getCFandPartnerSubjectSetting(session).getValue());
		mailEnt.setMessage(TextCoder.Encode(MailHelper.getInstance()
				.completeMessageForLocalization(
						BeansFactory.MailSettings()
								.getCFandPartnerTemplateSetting(session)
								.getValue(), name, surname, projectName)));
