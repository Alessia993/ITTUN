...
{
	public NewUserMailSender(InfoObject info)
	{
		super(info);
	}
	 ...
	 * @param info
	 */
	public NewUserMailSender(List<InfoObject> info)
	{
		super(info);
	}
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getNewUserSubjectSettings(session).getValue());
		mailEnt.setMessage(TextCoder
				.Encode(MailHelper.getInstance()
						.completeMessage(
								BeansFactory.MailSettings()
										.getNewUserTemplateSettings(session)
										.getValue(), name, surname, password,
								login)));
