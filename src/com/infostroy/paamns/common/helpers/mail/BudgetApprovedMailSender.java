 ...
 * 
 */
public class BudgetApprovedMailSender extends MailSender
{

	/**
	 * @param infoList
	 */
	public BudgetApprovedMailSender(List<InfoObject> infoList)
	{
		super(infoList);
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getBudgetApprovedSubjectSettings(session).getValue());
		mailEnt.setMessage(TextCoder.Encode(MailHelper.getInstance()
				.completeMessageForBudgetChanged(
						BeansFactory.MailSettings()
								.getBudgetApprovedTemplateSettings(session)
								.getValue(), denomination, projectName)));
