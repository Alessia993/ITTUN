 ...
 * 
 */
public class ValidationFlowSTCSendMailSender extends MailSender
{

	/**
	 * @param info
	 */
	public ValidationFlowSTCSendMailSender(List<InfoObject> info)
	{
		super(info);
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getValidationFlowSTCSendSubjectSettings(session).getValue());
		mailEnt.setMessage(TextCoder.Encode(MailHelper.getInstance()
				.completeMessageValidationFlow(
						BeansFactory
								.MailSettings()
								.getValidationFlowSTCSendTemplateSettings(
