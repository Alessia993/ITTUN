 ...
 * 
 */
public class ManagmentDurAGUAcceptMailSender extends MailSender
{

	/**
	 * @param info
	 */
	public ManagmentDurAGUAcceptMailSender(List<InfoObject> info)
	{
		super(info);
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getManagmentDurAGUAcceptSubjectSettings(session).getValue());
		mailEnt.setMessage(TextCoder.Encode(MailHelper.getInstance()
				.completeMessageManagmentDur(
						BeansFactory
								.MailSettings()
								.getManagmentDurAGUAcceptTemplateSettings(
