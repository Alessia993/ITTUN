 ...
 * 
 */
public class ValidationFlowAGUConfirmMailSender extends MailSender
{

	/**
	 * @param info
	 */
	public ValidationFlowAGUConfirmMailSender(List<InfoObject> info)
	{
		super(info);
			...
			MessagingException, PersistenceBeanException
	{
		...
		mailEnt.setSubject(BeansFactory.MailSettings()
				.getValidationFlowAGUConfirmSubjectSettings(session).getValue());
		mailEnt.setMessage(TextCoder.Encode(MailHelper.getInstance()
				.completeMessageValidationFlow(
						BeansFactory
								.MailSettings()
								.getValidationFlowAGUConfirmTemplateSettings(
