...
public class AdditionalFesrInfoListBean extends  EntityProjectListBean<AdditionalFesrInfo>
{
	...
	{	List<UserRoles> roles = this.getCurrentUser().getRoles();
		...
		List<AdditionalFesrInfo> list = BeansFactory.AdditionalFESRInfo().LoadByProjectCFPartnerAnagraphsAndTypeAndRole(this.getSelectedPartnerId(), this.getSelectedTransferType(), this.getProjectId(), null);//roleId);
		this.setList(list);
	...
	{
		this.goTo(PagesTypes.ADDITIONALFESRINFOEDIT);
	...
	{
		this.getSession().put("additionalFesrInfoEdit", this.getEntityEditId());
		this.goTo(PagesTypes.ADDITIONALFESRINFOEDIT);
